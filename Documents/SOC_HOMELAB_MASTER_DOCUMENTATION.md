...
| KPN Terugval | Panic button (2026-07-20) — forces this host's internet back onto the direct KPN connection if OPNsense ever has a problem; runs `scripts/network-fallback-to-kpn.sh` via Konsole. Also available as a button on the SOC Alarmdashboard itself. |
...
| 07-14 | Read-only OPNsense audit. Elastic Agent rolled out to the Bazzite host itself (reboot-resilience verified x2) and to WIN11-01 (+ Sysmon, SSH integrated) and ubuntu-server-01. ubuntu-server-01's long-standing `.100`-drift DHCP bug root-caused and fixed (MAC-based `dhcp-identifier`, see [§7.8-adjacent]). |
